...
import { Appointment, AppointmentStatus, Page } from '../models/appointment.model';
...
export class AppointmentService {
  ...
  getAllAppointments(page: number = 0, size: number = 5): Observable<Page<Appointment>> {
...
getAllPatients(page: number = 0, size: number = 100): Observable<Page<Patient>> {
  const params = new HttpParams()
    .set('page', page.toString())
    .set('size', size.toString());
  
  return this.http.get<Page<Patient>>('http://localhost:8092/api/patients', { params });
}


  getAllSpecialties(page: number = 0, size: number = 100): Observable<Page<Specialty>> {
  const params = new HttpParams()
    .set('page', page.toString())
    .set('size', size.toString());

  return this.http.get<Page<Specialty>>('http://localhost:8092/api/specialties', { params });
}


    getAvailableTimeSlots(date: Date, specialtyId: number): Observable<TimeSlot[]> {
  // Conversion de la Date en string au format YYYY-MM-DD
...
private formatDate(date: Date): string {
  ...
  getAppointmentsByStatus(status: AppointmentStatus, page: number = 0, size: number = 5): Observable<Page<Appointment>> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString());
    
    return this.http.get<Page<Appointment>>(`${this.apiUrl}/status/${status}`, { params });
  }

  getAppointmentsByPatientId(patientId: number, page: number = 0, size: number = 5): Observable<Page<Appointment>> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString());
    
    return this.http.get<Page<Appointment>>(`${this.apiUrl}/patient/${patientId}`, { params });
  }

  getAppointmentsByDate(date: string, page: number = 0, size: number = 5): Observable<Page<Appointment>> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString());
    
    return this.http.get<Page<Appointment>>(`${this.apiUrl}/date/${date}`, { params });
  }
  ...
  searchAppointments(
    ...
    if (criteria.timeSlotId) params = params.set('timeSlotId', criteria.timeSlotId.toString());
